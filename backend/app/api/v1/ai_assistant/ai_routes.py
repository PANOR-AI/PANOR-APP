...
from app.models.all_models import User, Patient, AIConversation, AuditLog, Prescription, LabReport, Vitals, EpidemiologyEvent
...
async def run_consultation(
    ...
    try:
        ...
        # 3b. Save EpidemiologyEvent if outbreak risk is detected
        epi_trace = next((t for t in result["traces"] if t["agent"] == "Epidemiology Agent"), None)
        if epi_trace:
            outbreak_risk = epi_trace["output"].get("outbreak_risk", "LOW")
            cluster_detected = epi_trace["output"].get("cluster_detected", False)
            if outbreak_risk.upper() in ("MEDIUM", "HIGH") or cluster_detected:
                db.add(EpidemiologyEvent(
                    disease_name=final_state.get("diagnosis", "Community-Acquired Pneumonia"),
